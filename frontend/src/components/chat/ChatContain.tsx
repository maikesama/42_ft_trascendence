...
export const ChatContain = (props: any) => {
    ...
    function renderDmsRow(props: any) {
        ...
        return (
            <ListItem style={style} key={index} onClick={event => changeChat('DM', dms[index]["partecipant"][0]["user"]?.userName, dms[index]?.id, dms[index]["partecipant"][0]["user"]?.img, dms[index]["partecipant"][0]["user"]?.idIntra, '')}>
    ...
    function renderSearchRow(props: any) {
        const { index, style } = props;
        return (
            <ListItem style={style} key={index} onClick={event => toDm(index)}>
